...
Because the repository still lacks a lockfile, `.github/workflows/generate-lockfile.yml` generates and validates it on Codex branches with Node 20.20.2 and commits only `package-lock.json` using `GITHUB_TOKEN`. No token or credential is embedded in the repository.
