...
تؤكد هذه الوثيقة ما ثبت في تشغيل Electron **معبأ فعلياً**، ولا ترفع الحالة إلى `PASS` لمجرد نجاح مسار B0–B9. دليل runtime الأحدث نجح (`runtimeResult=PASS`) على الالتزام السابق `5f7f65de84df6e0d46a0ddb9e585570f91a7396b`، لكنه **ليس دليلاً post-commit** للالتزام النهائي بعد تغييرات الإغلاق. يلزم إعادة التشغيل بعد الالتزام مع `trackedTreeClean=true` ومطابقة SHA قبل إصدار حكم نهائي.
...
| UI | `PASS` للمصفوفة | كل أداة عرضت `data-body-control` الصحيح، غيّرت raster، ثم أزالت الأثر بدقة. |
| الخلفية | `PASS` في B6 | 10,802,186 بكسل خلفية بعيدة بلا تغيير، max delta = 0 للخصر/اليدوي/Leg. |
| الرأس/المفاصل | `PARTIAL` | الرأس byte-identical؛ سُجلت 12 منطقة مفصلية لكل عملية. الأجزاء البعيدة التي لا يغطيها mask قد تتحرك في Leg، ولا يوجد بعد معيار continuity/displacement نهائي. |
| الاستمرارية | `PARTIAL` | لا توجد بعد بوابة هندسية مستقلة للـcontinuity أو حد displacement لكل مفصل. |
| الحفظ وإعادة الفتح | `PASS` لمسار B4 | hash النهائي مطابق؛ عملية جسم ويدوية وmask حفظت وأعيدت قراءتها. |
...
| الأداء | `PARTIAL` | قياس واحد كبير موجود؛ لا مصفوفة أحجام/cache/ذاكرة كاملة. |
| قبول Electron | `PASS` قبل الالتزام فقط | exe الحقيقي، `file://...app.asar...`، حارس الشبكة قبل أول نافذة. |
| الإجمالي | `PARTIAL` | لا دليل SHA نهائي نظيف ولا تغطية لكل بوابات الأداء/الحالات المطلوبة. |

## أدلة B0–B9 المعبأة

| الاختبار | النتيجة | تفاصيل قابلة للتحقق |
|---|---|---|
| B0 | `PASS` | `1 body detected locally · segmentation ready`، موضوع واحد، Pose محلي. |
| B1 | `PASS` | Waist غيّر full canvas عن baseline. |
| مصفوفة الجسم | `PASS` | الأدوات السبع التلقائية تغيّر البكسلات وتعود hash السابق تماماً بعد الإزالة؛ مع B1 Waist وB2 Manual تصبح الأدوات التسع مغطاة وظيفياً. |
| B2 | `PASS` | Manual Body Warp أنتج 16 stroke وتغيّر نهائي. |
| B3 | `PASS` | undo وredo أعادا hash الدقيق لمسار اليدوي. |
| B4 | `PASS` | save/reopen exact؛ وثّق project SHA وmask وعمليات الجسم/اليدوي. |
...
يعني نجاح Leg أن أثر الأداة ظهر في core مع بقاء الخلفية والرأس دون تغيير، لا أن كل نقطة limb يجب أن تتجمد؛ الهدف من Leg يتضمن حركة مشروعة في الأطراف. وتبقى بوابة منفصلة مطلوبة لتعريف حدود الاستمرارية المسموحة قبل حكم شامل على المفاصل.
...
2. توسيع قبول UI إلى تسلسل undo-all ثم redo-all لكل أدوات الجسم التسع، مع snapshots صادقة للحالات المتتالية.
3. إثبات source non-mutation وtwo-raster-layer isolation وbefore/after hash عبر مسار الإنتاج، فوق اختبار الوحدة الحالي.
4. إضافة instrumention تحليل حقيقي للـcache hit/miss وrequest IDs وإلغاء analysis القديم، مع مصفوفة حجم صغيرة ومتوسطة وكاملة وقياس الذاكرة.
5. تعريف وتطبيق بوابة continuity/displacement هندسية لكل مفصل؛ لا يصح استبدال ذلك بعبارة عامة أو بأرقام غير مقاسة.

## المراجع الداخلية

1. `_temp/live-evidence/retouch-phase3b-electron-acceptance.json` — دليل B0–B9 الحي للحزمة المعبأة، ما قبل الالتزام.
...
4. `artifacts/retouch-phase3b-performance.md` — حدود وقياسات الأداء الخام.
