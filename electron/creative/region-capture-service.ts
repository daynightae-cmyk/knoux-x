...
<div class="help">Drag: select · Drag inside: move · Handles: resize · Arrows: move · Shift+Arrows: resize · Enter: capture · Esc: cancel</div>
<script>
(() => {
...
export class RegionCaptureService {
  ...
  async capture(rawRequest: DesktopCaptureRequest): Promise<DesktopCaptureResult | null> {
    ...
    const normalized = sanitizedRectangle(selection, logicalBounds);
  ...
  private async selectRegion(display: Display, imageDataUrl: string, aspectPreset: RegionAspectPreset): Promise<Rectangle | null> {
    ...
    return new Promise<Rectangle | null>((resolve, reject) => {
      ...
      const complete = (event: IpcMainEvent, payload: SelectorPayload): void => {
        ...
        try {
          settle(sanitizedRectangle(payload, { x: 0, y: 0, width: display.bounds.width, height: display.bounds.height }));
        } catch (error) {
