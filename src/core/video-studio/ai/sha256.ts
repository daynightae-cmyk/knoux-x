...
/**
 * Synchronous SHA-256 over UTF-8 text. This deliberately avoids Node-only
 * crypto imports so deterministic plans can be created in the renderer too.
 */
export function sha256(text: string): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first = (h + sigma1 + choose + ROUND_CONSTANTS[index] + schedule[index]) >>> 0;
