    ...
    <div className="customization-settings-stack">
      <NeonPanel variant="dark" padding="lg">
        ...
        <div className="settings-two-columns">
          ...
          <label><span>{rtl ? 'الوضع' : 'Mode'}</span><NeonSelect value={settings.quickAccessToolbar.mode} onChange={(mode) => void updateSetting('quickAccessToolbar', { ...settings.quickAccessToolbar, mode: mode as ApplicationSettings['quickAccessToolbar']['mode'] })} options={[{ value: 'full', label: 'Full' }, { value: 'compact', label: 'Compact' }, { value: 'floating', label: 'Floating' }]} /></label>
          <label><span>{rtl ? 'الموقع' : 'Location'}</span><NeonSelect value={settings.quickAccessToolbar.location} onChange={(location) => void updateSetting('quickAccessToolbar', { ...settings.quickAccessToolbar, location: location as ApplicationSettings['quickAccessToolbar']['location'] })} options={[{ value: 'top', label: 'Top' }, { value: 'bottom', label: 'Bottom' }, { value: 'floating', label: 'Floating' }]} /></label>
          <label><span>{rtl ? 'الحجم' : 'Size'}</span><NeonSelect value={settings.quickAccessToolbar.size} onChange={(size) => void updateSetting('quickAccessToolbar', { ...settings.quickAccessToolbar, size: size as ApplicationSettings['quickAccessToolbar']['size'] })} options={[{ value: 'small', label: 'Small' }, { value: 'medium', label: 'Medium' }, { value: 'large', label: 'Large' }]} /></label>
        </div>
      ...
      <NeonPanel variant="dark" padding="lg">
        ...
        <div className="settings-two-columns">
          ...
          <label><span>{rtl ? 'الوضع' : 'Mode'}</span><NeonSelect value={settings.recordingToolbar.mode} onChange={(mode) => void updateToolbar({ mode: mode as ApplicationSettings['recordingToolbar']['mode'] })} options={[{ value: 'full', label: 'Full toolbar' }, { value: 'compact', label: 'Compact toolbar' }, { value: 'floating', label: 'Floating mini-controller' }]} /></label>
          <label><span>{rtl ? 'الحجم' : 'Control size'}</span><NeonSelect value={settings.recordingToolbar.size} onChange={(size) => void updateToolbar({ size: size as ApplicationSettings['recordingToolbar']['size'] })} options={[{ value: 'small', label: 'Small' }, { value: 'medium', label: 'Medium' }, { value: 'large', label: 'Large' }]} /></label>
          <label><span>{rtl ? 'الموقع' : 'Location'}</span><NeonSelect value={settings.recordingToolbar.location} onChange={(location) => void updateToolbar({ location: location as ApplicationSettings['recordingToolbar']['location'] })} options={[{ value: 'top', label: 'Top' }, { value: 'bottom', label: 'Bottom' }, { value: 'floating', label: 'Floating' }]} /></label>
      ...
      <NeonPanel variant="dark" padding="lg">
        ...
        <div className="settings-two-columns"><label><span>{rtl ? 'بحث' : 'Search'}</span><input type="search" value={shortcutSearch} onChange={(event) => setShortcutSearch(event.target.value)} /></label><label><span>{rtl ? 'الفئة' : 'Category'}</span><NeonSelect value={shortcutCategory} onChange={(category) => setShortcutCategory(category as typeof shortcutCategory)} options={[{ value: 'all', label: 'All' }, { value: 'playback', label: 'Playback' }, { value: 'capture', label: 'Capture' }, { value: 'recording', label: 'Recording' }, { value: 'editing', label: 'Editing' }, { value: 'workspace', label: 'Workspace' }, { value: 'file', label: 'File' }]} /></label></div>
      ...
      <NeonPanel variant="dark" padding="lg">
        ...
        <div className="settings-two-columns"><label><span>{rtl ? 'مساحة محفوظة' : 'Workspace preset'}</span><NeonSelect value={settings.workspace.selectedWorkspace} onChange={(workspace) => void applyWorkspacePreset(workspace)} options={[{ value: 'default', label: 'Default' }, ...settings.workspace.presets.map((preset) => ({ value: preset.id, label: preset.name }))]} /></label><label><span>{rtl ? 'حفظ المساحة الحالية' : 'Save current workspace'}</span><span className="customization-name-field"><input value={workspacePresetName} maxLength={100} onChange={(event) => setWorkspacePresetName(event.target.value)} /><button type="button" disabled={!workspacePresetName.trim()} onClick={() => void saveWorkspacePreset()}><Plus size={15} /></button></span></label></div>
        ...
        <div className="settings-two-columns"><label><span>{rtl ? 'آخر قسم' : 'Last section'}</span><NeonSelect value={settings.workspace.lastOpenedSection} onChange={(section) => void updateWorkspace({ lastOpenedSection: section as WorkspaceModuleId })} options={WORKSPACE_MODULES.filter((module) => !settings.workspace.hiddenModules.includes(module)).map((module) => ({ value: module, label: module }))} /></label><label><span>{rtl ? 'قسم مطوي' : 'Collapsed section'}</span><input value={settings.workspace.collapsedSections[0] ?? ''} onChange={(event) => void updateWorkspace({ collapsedSections: event.target.value ? [event.target.value] : [] })} /></label></div>
      </NeonPanel>
