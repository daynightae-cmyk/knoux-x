...
describe('installed slideshow verifier recovery boundaries', () => {
  test('reacquires the visible native dialog, UIA root, and controls with a bounded retry', () => {
    ...
    expect(source).toContain("$_.Class -eq 'Edit' -and $_.Id -eq 41477");
    expect(source).toContain('Send-DialogKey $addressEditor.Handle 0x73');
    expect(source).toContain('Type-Control $addressEditor.Handle $canonicalDirectory');
    expect(source).toContain('Click-DialogAddressGo $Handle');
    expect(source).toContain("$_.Current.Name -eq 'Address band toolbar'");
    expect(source).toContain('$attempt -le 3');
    expect(source).toContain("$_.Current.Name -match '^Address:\\s*'");
    expect(source).toContain('Native dialog directory mismatch after 3 attempts');
    expect(source).toContain('$payload = @($parsedPayload)');
  ...
  test('reconnects only to the same live installed process and normalizes uncertain input', () => {
    ...
    expect(inputSource).toMatch(
      /Click-CurrentPointer\r?\n\s+# Chromium may acknowledge the physical click/
    );
